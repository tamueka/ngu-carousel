...
export class NguCarousel<T> extends NguCarouselStore
  ...
  //   const context = new NguCarouselOutletContext<any>(nodeData);
  ...
  ngAfterContentInit() {
    ...
    //   this.buttonControl();
    // });
    // console.log('aftercontentinit', this._defDirec);
    this._observeRenderChanges();
  ...
  private _inputValidation() {
    ...
    this.velocity = this.inputs.velocity || this.velocity;
  ...
  private _onResizing(event: any): void {
    ...
    this.onResize = setTimeout(() => {
      if (this.deviceWidth !== event.target.outerWidth) {
        this._setStyle(this.carouselInner1.nativeElement, 'transition', ``);
        this._storeCarouselData();
      }
    }, 500);
  }
    ...
    if (this.inputs.touch) {
      ...
      hammertime.on('panend', (ev: any) => {
        if (Math.abs(ev.velocity) > this.velocity) {
          ...
          this._setStyle(this.carouselInner1.nativeElement, 'transform', '');
        }
      });
  ...
  private _carouselSize(): void {
    ...
    let dism = '';
    this.styleid = `.${this.token} > .ngucarousel > .ngucarousel-items`;
    ...
    if (this.vertical.enabled) {
      ...
      const itemWidth_xs =
        this.inputs.type === 'mobile'
          ? `${this.styleid} .item {flex: 0 0 ${95 / +this.inputs.grid.xs}%}`
          : `${this.styleid} .item {flex: 0 0 ${100 / +this.inputs.grid.xs}%}`;
      ...
      itemStyle = `${this.styleid} .item {flex: 0 0 ${this.inputs.grid.all}px}`;
    }

    this._renderer.addClass(this.carousel, this.token);
    if (this.vertical.enabled) {
      this._renderer.addClass(this.carouselInner1.nativeElement, 'nguvertical');
  ...
  private _carouselScrollOne(Btn: number): void {
    ...
    if (this.pointIndex === 1) {
      ...
      if (this.currentSlide === 0) {
        currentSlide = this._defDirec.length - this.items;
        itemSpeed = 400;
        this._btnBoolean(0, 1);
  ...
  private _carouselInterval(): void {
    if (this.interval && this.loop) {
      ...
      this.listener5 = this._renderer.listen(
        this.carouselMain1.nativeElement,
        'touchend',
        () => {
          this._carouselIntervalEvent(0);
      ...
      this.listener6 = this._renderer.listen(
        this.carouselMain1.nativeElement,
        'mouseenter',
        () => {
          this._carouselIntervalEvent(1);
      ...
      this.listener7 = this._renderer.listen(
        this.carouselMain1.nativeElement,
        'mouseleave',
        () => {
          this._carouselIntervalEvent(0);
